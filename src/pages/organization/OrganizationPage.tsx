...
import type { UserPermission, UserRole, UserScopeLevel, department, site, user, warehouse } from '../../entities/organization/types'
...
const scopeLabels: Record<UserScopeLevel, string> = {
  company: '회사 전체',
  site: '사업장',
  department: '부서',
}

const permissionsForRole = (role: UserRole): UserPermission[] => {
  if (role === 'totalAdmin') return ['organization.manage', 'purchase.request.create', 'purchase.order.manage', 'inventory.receipt.manage', 'inventory.issue.manage', 'approval.process']
  if (role === 'siteAdmin') return ['organization.manage', 'purchase.request.create', 'purchase.order.manage', 'inventory.receipt.manage', 'inventory.issue.manage']
  if (role === 'departmentAdmin') return ['purchase.request.create', 'approval.process']
...
export default function OrganizationPage({ initialTab = 'site' }: OrganizationPageProps) {
  ...
  const addUser = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setUsers((current) => [...current, {
      ...
      useYN: 'Y',
      deleteYN: 'N',
      permissions: permissionsForRole(userForm.roleId),
    }])
