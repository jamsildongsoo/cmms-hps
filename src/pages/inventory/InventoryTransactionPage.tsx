...
import type { InventoryTransactionReason, InventoryTransactionType } from '../../entities/inventory/types'
import type { PurchaseOrderResponse } from '../../entities/purchase-order/types'
import { getPurchaseOrders } from '../../features/purchase/api/purchaseApi'
import { createInventoryTransaction } from '../../features/inventory/api/inventoryApi'

type Props = { session: LoginInfo }
type TransactionTab = 'receipt' | 'issue'
type ReceiptReason = 'purchase-order-receipt' | 'general-receipt' | 'transfer-receipt'
type IssueReason = 'general-issue' | 'transfer-issue'
type TransactionItemForm = { materialId: string; materialName: string; quantity: string; unit: string }

const labels: Record<TransactionTab, string> = { receipt: '입고', issue: '출고' }
const reasonLabels: Record<InventoryTransactionReason, string> = { 'purchase-order-receipt': '구매오더 입고', 'general-receipt': '일반 입고', 'transfer-receipt': '창고이동 입고', 'general-issue': '일반 출고', 'transfer-issue': '창고이동 출고' }
...
export default function InventoryTransactionPage({ session }: Props) {
  ...
  const [issueReason, setIssueReason] = useState<IssueReason>('general-issue')
    ...
    <main className="page page-screen">
      ...
      <form id="inventory-transaction-form" onSubmit={submit}>
        <section className="card filter-card"><h2 className="card-title">{labels[activeTab]} 헤더정보</h2><div className="form-grid form-grid--4">
          ...
          <label className="field">처리유형<select value={activeTab === 'receipt' ? receiptReason : issueReason} onChange={(event) => changeReason(event.target.value as ReceiptReason | IssueReason)}>{activeTab === 'receipt' ? <><option value="purchase-order-receipt">{reasonLabels['purchase-order-receipt']}</option><option value="general-receipt">{reasonLabels['general-receipt']}</option><option value="transfer-receipt">{reasonLabels['transfer-receipt']}</option></> : <><option value="general-issue">{reasonLabels['general-issue']}</option><option value="transfer-issue">{reasonLabels['transfer-issue']}</option></>}</select></label>
