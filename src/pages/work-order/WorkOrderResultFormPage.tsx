...
import type { WorkOrder, WorkOrderResponse } from '../../entities/work-order/types'
...
const emptyWorkOrder = {
  ...
  status: '',
  items: [{ id: '', workName: '', workMethod: '', result: '' }],
} as WorkOrder
...
export default function WorkOrderResultFormPage({ onBack, mode = 'create', printInfo }: WorkOrderResultFormPageProps) {
  ...
  const emptyResult = { type: 'result' as const, workDate: '', workerId: '', manHours: '', manHoursUnit: '', cost: '', summary: '' }
  ...
  const save = () => setMessage('작업실적이 저장되었습니다. (파일럿)')
    ...
    <>
      ...
      <div className="card-grid card-grid--2">
        ...
        <section className="card">
          ...
          <label className="field">
            제목
            <input value={order.title} readOnly placeholder="작업오더 조회 결과" />
          </label>
      ...
      <section className="card">
        ...
        <div className="form-grid form-grid--5">
          <label className="field">작업일<input type="date" value={order.plan.workDate} readOnly /></label>
      ...
      <section className="card">
        ...
        <div className="form-grid form-grid--5">
          <label className="field">작업일<input type="date" value={order.result?.workDate ?? ''} onChange={(event) => updateResult('workDate', event.target.value)} readOnly={isView} /></label>
