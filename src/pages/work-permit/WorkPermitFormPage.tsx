...
const supplements: Array<[SupplementType, string]> = [
  ['highPlace', '고소작업'],
  ['electrical', '전기작업'],
  ['hotWork', '화기작업'],
...
const emptyGasInspections: GasInspection[] = Array.from({ length: 4 }, () => ({ gasName: '', result: '', checkedAt: '' }))
...
const createEmptyPermit = (): WorkPermit => ({
  id: '', permitName: '', permitType: '', equipmentId: '', equipmentName: '', workMethod: '', supervisorId: '', supervisorName: '',
  workPlace: '', permitFrom: '', permitTo: '', workSummary: '', safetyActionRequirements: createSafetyActions(),
  specialRequirements: '', safetyReviewOpinion: '', gasInspections: emptyGasInspections.map((item) => ({ ...item })),
  gasMeter: '', gasMeasurer: '', gasConfirmer: '', safetyCheckWitness: '', safetyCheckWitnessName: '',
      ...
      <div className="card-grid card-grid--2">
        <section className="card">
          ...
          <label className="field">
            허가명
            <input value={permit.permitName} onChange={(event) => update('permitName', event.target.value)} readOnly={isView} placeholder="허가명을 입력하세요" />
          </label>
          <label className="field field--spaced">
            개요
            <textarea value={permit.workSummary} onChange={(event) => update('workSummary', event.target.value)} readOnly={isView} rows={4} />
          </label>
          <label className="field field--spaced">
            허가번호
