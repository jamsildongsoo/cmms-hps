...
import type { PurchaseOrderResponse, PurchaseOrderSource } from '../../entities/purchase-order/types'
import type { PurchaseRequestResponse } from '../../entities/purchase-request/types'
import { createPurchaseOrder, getPurchaseOrders, getPurchaseRequests } from '../../features/purchase/api/purchaseApi'
...
const sourceLabels: Record<PurchaseOrderSource, string> = { 'purchase-request': '구매요청 전환', inventory: '재고기반 발주' }
...
export default function PurchaseOrderPage({ session }: Props) {
  ...
  useEffect(() => { void Promise.all([getPurchaseOrders(), getPurchaseRequests(), getMaterials()]).then(([orderData, requestData, materialData]) => { setOrders(orderData); setRequests(requestData); setMaterials(materialData) }) }, [])
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const created = await createPurchaseOrder({
      companyId: session.companyId, siteId: request?.siteId ?? session.siteId, departmentId: request?.departmentId ?? session.deptId, warehouseId, purchaserId: session.userId,
  ...
  const filteredOrders = orders.filter((item) => (!searchKeyword || !searchType || (searchType === 'number' ? item.id : searchType === 'title' ? item.name : item.departmentName).includes(searchKeyword)) && (!fromDate || item.orderDate >= fromDate) && (!toDate || item.orderDate <= toDate))
