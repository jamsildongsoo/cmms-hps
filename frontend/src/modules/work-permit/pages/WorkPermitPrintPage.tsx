import type { WorkPermit } from '../types/types'
import { generalRiskTemplate } from '../templates/general-risk'
    ...
    <section className="print-document">
      ...
      <table className="print-table print-report-table print-permit-header-table">
        ...
        <tbody>
          ...
          <tr><th>허가명</th><td>{permit.name || '-'}</td><th>허가기간</th><td>{formatDateTime(permit.permitFrom)} ~ {formatDateTime(permit.permitTo)}</td></tr>
          <tr><th>개요</th><td>{permit.summary || '-'}</td><th>작업지역</th><td>{permit.workPlace || '-'}</td></tr>
          ...
          <tr><td><div className="cell-grid cell-grid--3"><span>NG가스LEL</span><span>{permit.gasInspections[0]?.result || '-'}</span><span>{formatDateTime(permit.gasInspections[0]?.checkedAt || '')}</span></div></td><td className="text-xs">{permit.gasMeasurer || '-'}</td></tr>
        </tbody>
      </table>
