import type { PmRecordResponse } from '../types/types'
...
export default function PmRecordPrintPage({ record, companyName, deptName, userName, siteName, siteId }: Props) {
  return (
    <section className="print-document">
      ...
      <PrintCommonInfoTable equipmentId={record.equipmentId} equipmentName={record.equipmentName} maker={record.maker} model={record.model} id={record.id} title={record.name} summary={record.summary} />
      <table className="print-table print-table--fixed print-report-table print-phase-table">
        ...
        <tbody>
          <tr>
            <th>작업유형</th><td>{record.type || '-'}</td>
            <th>작업일</th><td>{record.date || '-'}</td>
          </tr>
          <tr>
            <th>작업자명</th><td>{record.workerName || '-'} ({record.workerId || '-'})</td>
            <th>결과</th><td>{record.decision || '-'}</td>
          </tr>
        </tbody>
      </table>
