import { PURCHASE_REQUEST_STATUS_LABELS as statusLabels } from '../../../../../shared/domain-codes'
import { useEffect, useState } from 'react'
import type { LoginInfo } from '../../auth/types/types'
import type { PurchaseRequestResponse } from '../types/types'
import { createPurchaseRequest, getPurchaseRequests } from '../../purchase-order/api/purchaseApi'
import type { MaterialResponse } from '../../material/types/types'
import { getMaterials } from '../../material/api/materialApi'
import { createApprovalDraft } from '../../approval/api/approvalApi'
import { buildApprovalBody } from '../../../shared/approval/approvalBody'
...
export default function PurchaseRequestPage({ session }: Props) {
  ...
  useEffect(() => { void Promise.all([getPurchaseRequests(), getMaterials()]).then(([requestData, materialData]) => { setRequests(requestData); setMaterials(materialData.items) }) }, [])
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const created = await createPurchaseRequest({
      companyId: session.companyId, siteId: session.siteId, deptId: session.deptId, requesterId: session.userId,
      name, requestDate: new Date().toISOString().slice(0, 10), requiredDate, deliveryConfirmed: false, purpose: name, remarks,
      items,
    })
    setRequests((current) => [...current, created]); void requestApproval(created); setIsAdding(false); resetForm()
  }
  ...
  const addRequestItem = () => setRequestItems((current) => [...current, { materialId: '', materialName: '', quantity: '', unit: 'EA' }])
  const removeRequestItem = (index: number) => setRequestItems((current) => current.filter((_, itemIndex) => itemIndex !== index))
  const filteredRequests = requests.filter((item) => (!searchKeyword || !searchType || (searchType === 'id' ? item.id : item.name).includes(searchKeyword)))
  const requestApproval = async (request: PurchaseRequestResponse) => { await createApprovalDraft({ module: 'purchase-request', recordId: request.id, title: `구매요청 승인 - ${request.name}`, content: buildApprovalBody([{ label: '요청번호', value: request.id }, { label: '요청명', value: request.name }, { label: '요청부서', value: request.departmentName }]), participants: [], submit: false }) }
