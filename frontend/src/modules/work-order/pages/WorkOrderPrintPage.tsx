import type { WorkOrderResponse } from '../types/types'
...
export default function WorkOrderPrintPage({ order, companyName, deptName, userName, siteName, siteId }: Props) {
  ...
  return (
    <section className="print-document">
      ...
      <PrintCommonInfoTable equipmentId={order.equipmentId} equipmentName={order.equipmentName} maker={order.maker} model={order.model} id={order.id} title={order.name} summary={plan.summary} />
      <table className="print-table print-table--fixed print-report-table print-phase-table">
        <tbody>
          <tr>
            <th>작업유형</th><td>{order.type || '-'}</td>
      ...
      <table className="print-table print-table--fixed print-report-table print-phase-table">
        ...
        <tbody>
          <tr>
            <th>계획</th><td>{plan.workerName || '-'} ({plan.workerId || '-'})</td><td>{plan.date || '-'}</td>
            <td>{plan.manHours || '-'} / {plan.manHoursUnit || '-'}</td><td>{plan.cost || '-'}</td>
          </tr>
          <tr>
            <th rowSpan={2}>실적</th><td>{result?.workerName || '-'} ({result?.workerId || '-'})</td><td>{result?.date || '-'}</td>
      ...
      <table className="print-table print-table--fixed print-item-table">
        ...
        <tbody>
          {order.items.map((item, index) => (
            <tr key={item.id || index}><td>{item.name || '-'}</td><td>{item.method || '-'}</td><td>{item.result || '-'}</td></tr>
          ))}
        </tbody>
      </table>
