...
import { getCodeItems } from '../../features/code/api/codeApi'
import type { CodeItem } from '../../entities/code/types'
...
import { createWorkOrder, updateWorkOrder } from '../../features/work-order/api/workOrderApi'
...
export default function WorkOrderPlanFormPage({ onBack, mode = 'create', printInfo }: WorkOrderFormPageProps) {
  ...
  useEffect(() => {
    void getCodeItems('workOrderType').then(setTypeItems).catch(() => setTypeItems([]))
  }, [])
  useEffect(() => {
  ...
  const save = async () => { if (!order.name.trim()) { setMessage('작업명을 입력해 주세요.'); return }; const body = { ...order, siteId: order.siteId || 'SITE-01', deptId: order.deptId || 'DEPT-01', plan: { ...order.plan, phase: 'plan' } }; if (mode === 'edit' && order.id) await updateWorkOrder(order.id, body); else await createWorkOrder(body); setMessage('작업오더가 저장되었습니다.') }
    ...
    <>
      ...
      <div className="card-grid card-grid--2">
        ...
        <section className="card">
          ...
          <label className="field">
            제목
            <input value={order.name} onChange={(event) => update('name', event.target.value)} placeholder="작업오더명을 입력하세요" readOnly={isView} />
          </label>
      ...
      <section className="card">
        ...
        <div className="form-grid form-grid--3">
          <label className="field">
            작업유형
            <select value={order.type} onChange={(event) => update('type', event.target.value)} disabled={isView}>
              <option value="">선택</option>{typeItems.filter((item) => item.active).map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}
            </select>
          </label>
          <label className="field">
            우선순위
      ...
      <section className="card">
        ...
        <div className="form-grid form-grid--5">
          <label className="field">
            작업일
            <input type="date" value={order.plan.date} onChange={(event) => updatePlan('date', event.target.value)} readOnly={isView} />
          </label>
          <label className="field">
            작업자 사번
            <div className="input-group">
          ...
          <table className="data-table">
            ...
            <tbody>
              {order.items.map((item, index) => (
                <tr key={index}>
                  {(['name', 'method'] as const).map((field) => (
                    <td key={field}>
                      <input value={item[field]} onChange={(event) => updateItem(index, field, event.target.value)} placeholder={field === 'name' ? '작업명 입력' : '작업방법 입력'} readOnly={isView} />
                    </td>
                  ))}
                  <td>
