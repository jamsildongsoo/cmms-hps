...
import { createPmRecord, updatePmRecord } from '../../features/pm-record/api/pmRecordApi'
...
export default function PmRecord({ onBack, mode = 'create', printInfo }: PmRecordFormPageProps) {
  ...
  useEffect(() => {
    void getCodeItems('inspectionType').then(setTypeItems).catch(() => setTypeItems([]))
  }, [])
  ...
  const save = async () => { if (!record.name.trim()) { setMessage('점검명을 입력해 주세요.'); return }; const body = { ...record, siteId: record.siteId || 'SITE-01', deptId: record.deptId || 'DEPT-01' }; if (mode === 'edit' && record.id) await updatePmRecord(record.id, body); else await createPmRecord(body); setMessage('점검기록이 저장되었습니다.'); }
      ...
      <section className="card">
        ...
        <div className="form-grid form-grid--5">
          <label className="field">
            ...
            <select
              value={record.type}
              onChange={(event) => updateRecord('type', event.target.value)}
              disabled={isView}
            >
          ...
          <label className="field">
            ...
            <input
              type="date"
              value={record.date}
              onChange={(event) => updateRecord('date', event.target.value)}
              readOnly={isView}
            />
          </label>
          <label className="field">
          ...
          <label className="field">
            ...
            <select
              ...
              onChange={(event) => updateRecord('decision', event.target.value)}
              disabled={isView}
            >
              <option value="">선택</option>
              <option>양호</option>
